...
- ディレクトリトラバーサル攻撃を防ぐ `validate_safe_path()` 関数を追加
- `--output`、`--inbox`、`--obsidian` CLI 引数を検証
- 安全なディレクトリ（~、.、./output、/tmp）へのファイル操作を制限

### DoS 防止
- `max_response_size`（10MB）と `max_content_length`（2MB）制限を追加
- Reddit、Twitter パーサーでレスポンスサイズチェック
- 無限のHTTPレスポンスによるメモリ枯渇を防止

### 入力検証
- GROQ_API_KEY 形式を検証（`gsk_`で始まる必要あり）
- 全パーサーでタイトル切り詰めを一貫して適用
