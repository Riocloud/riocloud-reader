...
## Обновления безопасности (v1.1.0)

### Защита от Path Traversal
- Добавлена функция `validate_safe_path()` для предотвращения атак обхода каталогов
- Проверка CLI-аргументов `--output`, `--inbox`, `--obsidian`
- Ограничение файловых операций безопасными директориями (~, ., ./output, /tmp)

### Защита от DoS
- Добавлены ограничения `max_response_size` (10MB) и `max_content_length` (2MB)
- Проверка размера ответа в парсерах Reddit и Twitter
- Предотвращение исчерпания памяти из-за неограниченных HTTP-ответов

### Проверка ввода
- Проверка формата GROQ_API_KEY (должен начинаться с `gsk_`)
- Согласованное усечение заголовков во всех парсерах
